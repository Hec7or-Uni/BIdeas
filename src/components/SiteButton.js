        ...
        <div className="mb-1">
            {more ? (
                <>
                    <button
                        type="button"
                        onClick={() => setActive(false) | (!isActive) ? setActiveMore(!isActiveMore) : setActiveMore(true)}
                        className={`flex items-center w-full ${isActive ? 'px-1 py-2' : 'px-2 py-2'} rounded-md hover:bg-gray-200 relative`}
                    >
                        {icon}
                        {!isActive && (
                            <>
                    ...
                    <div className="flex flex-col w-full">
                        {isActiveMore && !isActive && (links.map(item => {
                            return (
                                <Link href={item.url}>
